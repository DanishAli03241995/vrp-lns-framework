"""
Time-aware LNS-SA engine for Random Removal + Regret-2 Insertion.

This engine is designed for the fixed depot timing variant after split repair.
It keeps the original Random Removal + Regret-2 Insertion idea, but makes the
repair step time-aware by allowing only insertion positions that keep:
- vehicle capacity feasible;
- route structure valid;
- fixed depot departure feasible;
- route finish time within the working day.

Scope:
- fixed depot-ready time only;
- no dispatch waves;
...
# =====================================================
# Fixed timing feasibility helpers
# =====================================================


def evaluate_fixed_timing_route(
    route,
    route_id,
    demand,
    vehicle_capacity,
    ...
    """Evaluate one depot-origin route under fixed depot timing."""
    route_distance = compute_route_distance(route, routing_distance)
    ...
    return {
        ...
        "route_service_time_hours": compute_route_service_time_hours(
            route,
            service_time,
        ),
...
def evaluate_fixed_timing_solution(
    solution,
    demand,
    vehicle_capacity,
    routing_distance,
...
def summarize_fixed_timing_records(records):
    """Summarize route-level timing records."""
    if not records:
        return {
            "timing_model": "fixed_depot_ready_time_lns",
    ...
    avg_duration = (
        sum(record["route_duration_hours"] for record in records)
        / len(records)
    )
    avg_utilization = (
        sum(record["route_utilization"] for record in records)
        / len(records)
    )
...
def is_route_fixed_timing_feasible(
    route,
    demand,
    vehicle_capacity,
    routing_distance,
    ...
    """Return True only if one route satisfies capacity and timing."""
    record = evaluate_fixed_timing_route(
        route=route,
        route_id=1,
        demand=demand,
        vehicle_capacity=vehicle_capacity,
...
def find_time_feasible_insertions(
    ...
    """Find all capacity- and timing-feasible positions for one customer."""
    feasible_insertions = []
    ...
    for route_index, trip in enumerate(solution):
        ...
        for insert_position in range(1, len(trip)):
            ...
            if not is_route_fixed_timing_feasible(
    ...
    if allow_new_route:
        ...
        if is_route_fixed_timing_feasible(
            route=new_route,
            demand=demand,
            vehicle_capacity=vehicle_capacity,
            routing_distance=routing_distance,
...
def regret_2_insertion_fixed_timing(
    partial_solution,
    removed_customers,
    demand,
    vehicle_capacity,
    ...
    """Reinsert removed customers using time-aware regret-2 insertion."""
...
def validate_fixed_timing_solution(
    solution,
    expected_customers,
    demand,
    vehicle_capacity,
    ...
    """Validate customer coverage, capacity, and fixed timing."""
    records = evaluate_fixed_timing_solution(
        solution=solution,
        demand=demand,
        vehicle_capacity=vehicle_capacity,
        routing_distance=routing_distance,
    ...
    timing_summary = summarize_fixed_timing_records(records)
...
# =====================================================
# Main fixed-timing random + regret LNS-SA engine
# =====================================================


def run_lns_sa_random_regret_fixed_timing(
    initial_solution,
    demand,
    vehicle_capacity,
    routing_distance,
    ...
    """Run time-aware LNS-SA using random removal and regret-2 insertion."""
    random.seed(seed)
    ...
    initial_validation = validate_fixed_timing_solution(
    ...
    if not initial_validation["solution_feasible"]:
        raise ValueError(
            "Initial fixed-timing LNS solution is not feasible. "
            "Run fixed timing split repair before LNS."
        )
    ...
    for iteration in range(n_iterations):
        ...
        insertion_result = regret_2_insertion_fixed_timing(
        ...
        candidate_validation = validate_fixed_timing_solution(
        ...
        records.append({
            ...
            "operator_pair": "random_regret_fixed_timing",
            "destroy_operator": "random_removal",
            "repair_operator": "regret_2_insertion_fixed_timing",
            "removed_customers": removed_customers,
            "candidate_distance": candidate_distance,
            "current_distance": current_distance,
...
if __name__ == "__main__":
    ...
    best, distance, run_summary = run_lns_sa_random_regret_fixed_timing(
