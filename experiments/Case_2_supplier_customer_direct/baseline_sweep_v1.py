# Experiment entry point: Supplier -> Customer baseline using sweep clustering
...
def run_experiment(config):
    ...
    for supplier_id, subproblem in supplier_subproblems.items():
        ...
        # Intent: Build supplier-local sweep clusters around this supplier origin.
        # This creates the cluster list; the later cluster-data call only extracts coordinates
        # and demand for each cluster.
        ...
        for cluster in clusters:
            cluster_customer_cord, cluster_demand = sweep_clustering_v1.get_cluster_data(
                cluster,
                supplier_customer_cord,
                supplier_demand,
            )
    ...
    results_path = create_experiment_folder(
        "supplier_customer_only_baseline_sweep_v1",
        INSTANCE_NAME,
    )
    ...
    metrics = {
        "algorithm": "supplier_customer_only_baseline_sweep_v1",
        "instance": INSTANCE_NAME,
        "seed": 42,
        "construction": "angular_partition + sweep_clustering + nearest_neighbor + 2-opt",
    ...
    plot_supplier_routes(
        ...
        title="Supplier-Level Routes - Angular Partition + Sweep + 2-Opt + Supplier-Level Relocation",
    )
    ...
    plot_supplier_routes(
        ...
        title="Supplier-Level Routes - Angular Partition + Sweep + 2-Opt + Supplier-Level Relocation + 2-Opt",
    )
