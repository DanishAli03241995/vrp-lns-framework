...
Loads the latest supplier-customer direct no-cluster baseline run, applies
...
def run_lns_sa_experiment(
    ...
    # The cleaned no-cluster baseline saves config_used.json, but the LNS step
    # starts from metrics.json routes, so metrics.json remains sufficient.
    ...
    lns_metrics = {
        "algorithm": "supplier_customer_lns_sa_no_cluster_v1",
    ...
    plot_supplier_routes(
        ...
        title="Case 2 Direct No-Cluster + LNS-SA Routes",
    )
