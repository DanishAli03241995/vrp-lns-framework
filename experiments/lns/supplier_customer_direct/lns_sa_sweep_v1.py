...
Loads the latest supplier-customer direct sweep baseline run, rebuilds the
instance, applies supplier-level LNS-SA, and stores improved route outputs in
the same run folder.
"""
...
def run_lns_sa_experiment(
    ...
    # Some older baseline sweep runs did not save config_used.json. The LNS
    # wrapper only needs coordinates/capacity, so metrics.json is enough.
    ...
    lns_metrics = {
        "algorithm": "supplier_customer_lns_sa_sweep_v1",
    ...
    plot_supplier_routes(
        ...
        title="Case 2 Direct Sweep + LNS-SA Routes",
    )
