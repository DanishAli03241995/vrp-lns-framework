"""
Case 3: Hybrid Supplier-Customer + No Internal Clustering + LNS-SA.

Loads the latest hybrid no-cluster baseline run, applies LNS-SA separately to:
1. depot-side customer routes as one shared depot pool
2. supplier-direct routes per supplier
and stores improved route outputs in the same run folder.
"""
...
def compute_supplier_depot_replenishment(
    ...
    for record in post_reloc_2opt_route_records:
        ...
        supplier_id = int(record["supplier_region_id"])
...
def run_lns_sa_experiment(
    ...
    if depot_initial_solution:
        ...
        depot_best_solution, depot_best_distance = run_basic_lns_sa(
    ...
    for supplier_id, supplier_data in supplier_metrics.items():
        ...
        supplier_best_solution, supplier_best_distance = run_basic_lns_sa(
    ...
    lns_metrics = {
        "algorithm": "hybrid_supplier_customer_lns_sa_no_cluster_v1",
    ...
    plot_supplier_routes(
        ...
        title="Case 3 Hybrid No-Cluster + LNS-SA Routes",
        depot_cord=depot_cord,
    )

    print("\n===================================")
    print("HYBRID LNS-SA COMPLETE")
    print("===================================")
