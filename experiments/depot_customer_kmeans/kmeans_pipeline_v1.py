"""Basic depot-customer KMeans experiment pipeline."""
...
kmeans = import_module("construction.02_k_means_clustering_v1")

DEFAULT_INSTANCE_MODULE = "instances.toy_instance_v1_40c"
DEFAULT_RESULT_DIR = Path("results/depot_customer_kmeans")
ALGORITHM_NAME = "generated_depot_customer_kmeans_initial_pipeline"
...
def estimate_k_from_capacity(demand, vehicle_capacity):
    total_demand = sum(demand.values())

    return max(1, int((total_demand + vehicle_capacity - 1) // vehicle_capacity))


def build_kmeans_clusters(customer_cord, demand, vehicle_capacity, K=None):
    if K is None:
        K = estimate_k_from_capacity(demand, vehicle_capacity)

    clusters = kmeans.kmeans_clustering(customer_cord, K)
...
def run_cluster_routing(
    ...
    **routing_kwargs,
    ...
    for cluster in clusters:
        cluster_customer_cord, cluster_demand = kmeans.get_cluster_data(
            cluster,
            customer_cord,
            demand,
        )
...
def run_kmeans_nn_experiment(
    ...
    selected_k, clusters = build_kmeans_clusters(
        customer_cord,
        demand,
        vehicle_capacity,
        K=K,
    ...
    return {
        "algorithm": "depot_customer_kmeans_nn_v1",
        "K": selected_k,
        "clusters": clusters,
...
def run_kmeans_nn_2opt_experiment(
    depot_cord,
    customer_cord,
    demand,
    vehicle_capacity,
    ...
    nn_result = run_kmeans_nn_experiment(
        depot_cord,
        customer_cord,
        demand,
        vehicle_capacity,
    ...
    return {
        **nn_result,
        "algorithm": "depot_customer_kmeans_nn_2opt_v1",
...
def run_kmeans_nn_2opt_relocation_2opt_experiment(
    depot_cord,
    customer_cord,
    demand,
    vehicle_capacity,
    ...
    two_opt_result = run_kmeans_nn_2opt_experiment(
        depot_cord,
        customer_cord,
        demand,
        vehicle_capacity,
    ...
    return {
        **two_opt_result,
        "algorithm": "depot_customer_kmeans_nn_2opt_relocation_2opt_v1",
        "construction": "k_means_clustering + nearest_neighbor + 2-opt",
...
def run_from_instance(instance_module_path=DEFAULT_INSTANCE_MODULE, K=None, depot_id=0):
    instance_data = load_instance_data(instance_module_path)

    result = run_kmeans_nn_experiment(
...
def run_final_from_instance(instance_module_path=DEFAULT_INSTANCE_MODULE, K=None, depot_id=0):
    instance_data = load_instance_data(instance_module_path)

    result = run_kmeans_nn_2opt_relocation_2opt_experiment(
        instance_data["depot_cord"],
        instance_data["customer_cord"],
        instance_data["demand"],
        instance_data["vehicle_capacity"],
...
def build_generated_metrics(config, instance, result, routes_by_stage):
    ...
    metrics = {
        ...
        "construction": "kmeans_clustering + nearest_neighbor",
        "local_search": "2-opt + global 1-0 relocation + 2-opt",
...
def run_generated_experiment(config):
    ...
    for stage_name, routes in routes_by_stage.items():
        ...
        plot_routes(
            ...
            title=f"{label} - kmeans - {stage_name}",
        )
...
def print_experiment_summary(result):
    print("==================================")
    print("DEPOT-CUSTOMER KMEANS SUMMARY")
    print("==================================")
...
if __name__ == "__main__":
    for experiment_config in build_generated_experiments():
        print("\n===================================")
        print("Running generated depot-customer KMeans experiment:")
