...
# Hybrid Supplier-Customer KMeans baseline with wave-aware depot dispatch construction
...
from utils import depot_timing_wave_constructed_utils
...
def run_experiment(config):
    ...
    depot_customer_wave_assignment_summary = (
        depot_timing_wave_constructed_utils.summarize_customer_wave_assignment(
            [],
            dispatch_waves,
        )
    )
    ...
    for supplier_id, subproblem in supplier_subproblems.items():
        ...
        supplier_dispatch_wave_timing_summary = (
            depot_timing_wave_constructed_utils.summarize_dispatch_wave_timing([])
        )
    ...
    if len(global_depot_customer_cord) > 0:
        ...
        ) = depot_timing_wave_constructed_utils.generate_depot_customer_wave_times(
        ...
        (
            depot_customers_by_wave,
            depot_customers_without_feasible_wave,
        ) = depot_timing_wave_constructed_utils.group_customer_records_by_dispatch_wave(
            depot_customer_wave_timing_records,
            dispatch_waves,
        )
        ...
        for dispatch_wave in sorted(dispatch_waves):
            ...
            wave_cluster_record = {
                ...
                "dispatch_wave_label": (
                    depot_timing_wave_constructed_utils.format_hour(
                        dispatch_wave
                    )
                ),
            ...
            wave_timing_records = (
                depot_timing_wave_constructed_utils.evaluate_wave_constructed_routes(
            ...
            wave_timing_summary_key = (
                ...
                + depot_timing_wave_constructed_utils.format_hour(dispatch_wave)
            )
    ...
    results_path = create_experiment_folder(
        "hybrid_supplier_customer_kmeans_timing_waves_constructed_v1",
        INSTANCE_NAME,
    )
    ...
    depot_timing_summary = depot_timing_wave_constructed_utils.summarize_dispatch_wave_timing(
        depot_timing_route_records
    )

    metrics = {
        "algorithm": "hybrid_supplier_customer_kmeans_timing_waves_constructed_v1",
        ...
        "supplier_arrival_start_time_label": (
            depot_timing_wave_constructed_utils.format_hour(supplier_arrival_start_time)
        ),
        "supplier_arrival_end_time": supplier_arrival_end_time,
        "supplier_arrival_end_time_label": (
            depot_timing_wave_constructed_utils.format_hour(supplier_arrival_end_time)
        ),
        "depot_handling_time": depot_handling_time,
        "depot_handling_time_minutes": int(round(depot_handling_time * 60)),
        "dispatch_waves": dispatch_waves,
        "dispatch_wave_labels": [
            depot_timing_wave_constructed_utils.format_hour(wave)
            for wave in dispatch_waves
        ],
        "working_day_end_time": working_day_end_time,
        "working_day_end_time_label": (
            depot_timing_wave_constructed_utils.format_hour(working_day_end_time)
        ),
    ...
    plot_supplier_routes(
        ...
        customer_cord,
        results_path,
        filename="route_plot_dispatch_wave_constructed.png",
        title=(
            "Supplier-Level Routes - Hybrid + KMeans + "
            "Dispatch-Wave Constructed Depot Routes"
        ),
        depot_cord=depot_cord,
    )
