...
# Hybrid Supplier-Customer KMeans baseline with dispatch-wave depot timing evaluation
...
from utils import depot_timing_wave_utils
...
def run_experiment(config):
    ...
    # Dispatch-wave depot timing outputs. These are only for depot-origin routes.
    ...
    for supplier_id, subproblem in supplier_subproblems.items():
        ...
        supplier_dispatch_wave_timing_summary = (
            depot_timing_wave_utils.summarize_dispatch_wave_timing([])
        )
    ...
    if len(global_depot_customer_cord) > 0:
        ...
        ) = depot_timing_wave_utils.generate_depot_customer_wave_times(
        ...
        global_depot_timing_records = (
            depot_timing_wave_utils.evaluate_dispatch_wave_routes(
    ...
    results_path = create_experiment_folder(
        "hybrid_supplier_customer_kmeans_timing_waves_v1",
        INSTANCE_NAME,
    )
    ...
    depot_timing_summary = depot_timing_wave_utils.summarize_dispatch_wave_timing(
        depot_timing_route_records
    )

    metrics = {
        "algorithm": "hybrid_supplier_customer_kmeans_timing_waves_v1",
        ...
        "supplier_arrival_start_time_label": (
            depot_timing_wave_utils.format_hour(supplier_arrival_start_time)
        ),
        "supplier_arrival_end_time": supplier_arrival_end_time,
        "supplier_arrival_end_time_label": (
            depot_timing_wave_utils.format_hour(supplier_arrival_end_time)
        ),
        "depot_handling_time": depot_handling_time,
        "depot_handling_time_minutes": int(round(depot_handling_time * 60)),
        "dispatch_waves": dispatch_waves,
        "dispatch_wave_labels": [
            depot_timing_wave_utils.format_hour(wave)
            for wave in dispatch_waves
        ],
        "working_day_end_time": working_day_end_time,
        "working_day_end_time_label": (
            depot_timing_wave_utils.format_hour(working_day_end_time)
        ),
