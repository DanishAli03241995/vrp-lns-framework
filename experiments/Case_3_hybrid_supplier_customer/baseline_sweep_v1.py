...
# Hybrid Supplier-Customer baseline using angular partition + sweep clustering
...
sweep_clustering_v1 = importlib.import_module(
    "construction.01_sweep_clustering_v1"
)
...
def run_experiment(config):
    ...
    depot_customer_supplier_map = {}
    all_depot_customer_demand = {}

    # ======================================================
    # System 2 routing:
    # For each supplier -> hybrid split -> route locally
    # ======================================================
    ...
    for supplier_id, subproblem in supplier_subproblems.items():
        ...
        for customer_id, coord in supplier_customer_cord.items():
            ...
            if customer_demand >= direct_delivery_threshold:
                ...
                all_depot_customer_demand[customer_id] = customer_demand
        ...
        depot_routing_distance = lambda i, j: dynamic_euc_distance(
            i,
            j,
            depot_cord,
            depot_customer_cord,
        ...
        depot_routes = []
        depot_two_opt_routes = []
        depot_relocation_routes = []
        depot_post_reloc_2opt_routes = []

        # ==================================================
        # Supplier-direct sweep clustering
        # ==================================================

        angle_data = sweep_clustering_v1.compute_sweep_order(
            supplier_origin,
            supplier_direct_customer_cord,
            supplier_direct_demand,
        )

        clusters = sweep_clustering_v1.form_sweep_clusters(
            angle_data,
            vehicle_capacity,
        )
        ...
        # ==================================================
        # Depot-side routing for small-demand customers
        # ==================================================

        if len(depot_customer_cord) > 0:
            depot_angle_data = sweep_clustering_v1.compute_sweep_order(
                depot_cord,
                depot_customer_cord,
                depot_customer_demand,
            )

            depot_clusters = sweep_clustering_v1.form_sweep_clusters(
                depot_angle_data,
                vehicle_capacity,
            )

            for depot_cluster in depot_clusters:
                depot_cluster_customer_cord, depot_cluster_demand = (
                    sweep_clustering_v1.get_cluster_data(
                        depot_cluster,
                        depot_customer_cord,
                        depot_customer_demand,
                    )
                ...
                for trip in depot_two_opt_route:
                    ...
                    for c in range(len(trip) - 1):
                        trip_distance += depot_routing_distance(
                            trip[c],
                            trip[c + 1],
                        )
                ...
                baseline_cluster_capacity_flags.append(
                    depot_capacity_feasibility
                )
                baseline_cluster_structural_flags.append(
                    depot_structural_validity
                )
                ...
                for trip in depot_route:
                    ...
                    for customer in trip[1:-1]:
                        trip_load += depot_customer_demand[customer]
                ...
                depot_routes.extend(depot_route)
                depot_two_opt_routes.extend(depot_two_opt_route)

            depot_relocation_routes = (
                baseline_nn_2opt_relocation_2opt_v1.run_relocation(
                    copy.deepcopy(depot_two_opt_routes),
                    depot_customer_demand,
                    vehicle_capacity,
                    depot_routing_distance,
                )
            ...
            depot_post_reloc_2opt_routes = (
                baseline_nn_2opt_relocation_2opt_v1.run_2opt(
                    copy.deepcopy(depot_relocation_routes),
                    depot_routing_distance,
                )
            )

            for trip in depot_relocation_routes:
                trip_distance = 0
            ...
            for trip in depot_post_reloc_2opt_routes:
                trip_distance = 0
        ...
        for trip in depot_routes:
            baseline_route_records.append({
                "origin_type": "depot",
                "supplier_region_id": supplier_id,
                "trip": trip,
            })
        ...
        for trip in depot_two_opt_routes:
        ...
        all_two_opt_routes.extend(depot_two_opt_routes)
        all_two_opt_routes.extend(supplier_two_opt_routes)

        all_relocation_routes.extend(depot_relocation_routes)
        all_relocation_routes.extend(supplier_relocation_routes)

        all_post_reloc_2opt_routes.extend(depot_post_reloc_2opt_routes)
    ...
    results_path = create_experiment_folder(
        "hybrid_supplier_customer_sweep_v1",
        INSTANCE_NAME,
    )
    ...
    (
        ...
        depot_customer_supplier_map=depot_customer_supplier_map,
        depot_customer_demand=all_depot_customer_demand,
    ...
    metrics = {
        "algorithm": "hybrid_supplier_customer_sweep_v1",
        ...
        "construction": (
            "angular_partition + sweep_clustering + nearest_neighbor + 2-opt"
        ),
        "local_search": (
    ...
    plot_supplier_routes(
        ...
        title="Supplier-Level Routes - Angular Partition + Sweep + Baseline NN",
        depot_cord=depot_cord,
    )
    ...
    plot_supplier_routes(
        ...
        title="Supplier-Level Routes - Angular Partition + Sweep + Baseline NN + 2-Opt",
        depot_cord=depot_cord,
    )
    ...
    plot_supplier_routes(
        ...
        title=(
            "Supplier-Level Routes - Angular Partition + Sweep + "
