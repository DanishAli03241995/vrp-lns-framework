# Hybrid + angular partitioning + no internal clustering
...
def run_experiment(config):
    ...
    for supplier_id, subproblem in supplier_subproblems.items():
        ...
        # ==================================================
        # Supplier-direct routing without internal clustering
        # ==================================================

        clusters = []

        if len(supplier_direct_customer_cord) > 0:
            clusters = [list(supplier_direct_customer_cord.keys())]
        ...
        if len(depot_customer_cord) > 0:
            depot_clusters = [list(depot_customer_cord.keys())]

            for depot_cluster in depot_clusters:
                depot_cluster_customer_cord = depot_customer_cord
                depot_cluster_demand = depot_customer_demand
        ...
        for cluster in clusters:
            cluster_customer_cord = supplier_direct_customer_cord
            cluster_demand = supplier_direct_demand
    ...
    results_path = create_experiment_folder(
        "hybrid_supplier_customer_no_cluster_v1",
        INSTANCE_NAME,
    )
    ...
    metrics = {
        "algorithm": "hybrid_supplier_customer_no_cluster_v1",
        ...
        "construction": (
            "angular_partition + nearest_neighbor + 2-opt"
        ),
        "local_search": (
    ...
    plot_supplier_routes(
        ...
        title=(
            "Supplier-Level Routes - Angular Partition + "
            "No Internal Clustering + Baseline NN"
        ),
        depot_cord=depot_cord,
    )
    ...
    plot_supplier_routes(
        ...
        title=(
            "Supplier-Level Routes - Angular Partition + "
            "No Internal Clustering + Baseline NN + 2-Opt"
        ),
        depot_cord=depot_cord,
    )
